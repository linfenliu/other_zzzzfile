...
import solids;
currentlight=(5,5,5);
currentprojection = orthographic(5.22461008444299,3.4406452310546,2.53558312844022);
size(450);
xaxis3(scale(1.3)*Label("$x$"), p=red+opacity(0.95), -1.5, 1.5, Arrow3(DefaultHead3,15));
yaxis3(scale(1.3)*Label("$y$"), p=green+opacity(0.95), -1.5,1.5,Arrow3(DefaultHead3,15));
zaxis3(scale(1.3)*Label("$z$"), p=blue+opacity(0.95), -0.5,2,Arrow3(DefaultHead3,15));

path units = unitsquare;
path3 units_3 = shift(-0.5,-0.5,0)* path3(units);


path3 u0 = units_3;
triple[] ps;
ps[0] = (0,0,0);
draw(surface(u0), surfacepen = lightblue+opacity(0.7), meshpen = thick()+black+2);
for (int i = 1; i < 15; ++i)
{
	ps[i] = i*(-0.02,0.02,0.1);
	path3 u1 = rotate(0.2*i, ps[i] ,ps[i]+Z) * shift(-0.02,0.02,0.1) * u0; 
	
	dot(ps[i], red+2);
	draw(u1, p=black+thick());
	
	draw(extrude(u0, u1), surfacepen= lightblue+opacity(0.7));
	for(int j = 0; j < 4; ++j)
	{
		draw(relpoint(u0, j/4)--relpoint(u1, j/4), thick()+black+2);
	}
...
int n = floor(ps.length/2);
triple a0 = ps[n];
triple a1 = (0,0.8,0)+a0;
draw(Label("中心参数线", BeginPoint),a1--a0, black+1, EndArrow3(DefaultHead2,10)); 


Label matrix = ("$\Biggl[ \begin{tabular}{cccc}
u2x & v2x & n2x & 0 \\
u2y & v2y & n2y & 0 \\ 
u2z & v2z & n2z & 0 \\
0	& 0   & 0	& 1 \\
\end{tabular} \Biggr]$");
label(matrix, (-2,-2,0.5));
